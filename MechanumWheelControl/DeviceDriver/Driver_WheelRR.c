...
#define MOTOR_PWM_TOUTMAP             IfxGtm_TOM1_6_TOUT24_P33_2_OUT         /* LED_PWM which will be driven by the PWM          */
#define MOTOR_PWM                     &MODULE_P33,2
#define MOTOR_DIR                     &MODULE_P33,1

/*****************************/
/***** encoder parameter *****/
/*****************************/
#define ENCODER_CHA_REQ_IN        (&IfxScu_REQ12_P11_10_IN)
#define INTERRUPT_TRIGGER_CHANNEL   (IfxScuEru_InputNodePointer_3)    /* same with statement below  */
#define OUTPUT_CHANNEL              (IfxScuEru_OutputChannel_3)       /* same with statement above  */

#define ENCODER_CHA                 &MODULE_P11,10
#define ENCODER_CHB                 &MODULE_P33,0
...
static inline void set_motorRR_dutycycle(float32 dutycycle); // dutycycle : 100f ~ 0f

static inline void set_motorRR_direction(boolean clock_wise); // 1 : CW, 0: CCW

// 0% ~ 100%
static inline float32 get_motorRR_speed(void);

// 1 : CW, 0 : CCW
static inline boolean get_motorRR_direction(void);

static inline sint32 get_encoderRR_tick(void);
...
void set_wheelRR_dutycycle(float32 dutycycle) { // dutycycle : 100f ~ -100f
    set_motorRR_dutycycle(dutycycle);

#if MOTOR_FORWARD_DIR == 1
    if(dutycycle >= 0) {
        set_motorRR_direction(1);
    } else {
        set_motorRR_direction(0);
    }
#else
    if(dutycycle >= 0) {
        set_motorRR_direction(0);
...
float32 get_wheelRR_dutycycle(void) {
    if(get_wheelRR_direction() == 1)
        return get_wheelRR_speed();
    else
        return -get_wheelRR_speed();
}
...
float32 get_wheelRR_speed(void) {
    return get_motorRR_speed();
}

// 1 : forward, 0 : backward
boolean get_wheelRR_direction(void){
#if MOTOR_FORWARD_DIR == 1
    return get_motorRR_direction();
#else
    return !(get_motorRR_direction());
#endif
...
void init_motorRR(void) {
    ...
    set_motorRR_dutycycle(0);
    set_motorRR_direction(TRUE);
}


static inline void set_motorRR_dutycycle(float32 dutycycle){ // 0% ~ 100%
...
void init_encoderRR(void) {
    ...
    IfxSrc_init(src, IfxSrc_Tos_cpu0, ISR_PRIORITY_SCUERU3);
    IfxSrc_enable(src);
}
